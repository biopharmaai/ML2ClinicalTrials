...
def split_data(data, target, test_size):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data, label, test_size=test_size, random_state=123, shuffle=True
    )
...
def quantile_transform(X_train, X_valid, X_test):
    ...
    qt = QuantileTransformer(random_state=55688, output_distribution="normal").fit(
        quantile_train
    )
...
def mortality_rate(phase):
    ...
    multihot_feature = [
        c for c in X_train.columns if "MaskingType-" in c or "ipd_info_type-" in c
    ]  # "MaskingType-*"(0/1) + ipd_info_type-*(0/1)
    int_feature = ["enrollment", "number_of_arms"] + [
        c for c in X_train.columns if "Number" in c or "masking_num" in c
    ]  # "*arm number", "intervention number"
    age_feature = [
    ...
    numerical_features = [
        c for c in X_train.columns if c in cat_features + int_feature + age_feature
    ]
    ...
    train_criteria_lst = (
        X_train["eligibility/criteria/textblock"].fillna("unknown").tolist()
    )
    ...
    train_dataset = Trial_Dataset_tabular(
        train_nctid_lst,
        train_label_lst,
        train_smiles_lst,
        train_icdcode_lst,
    ...
    train_loader = DataLoader(
        ...
        batch_size=32,
        shuffle=True,
        collate_fn=trial_tabular_2_collate_fn,
    )
    ...
    valid_criteria_lst = (
        X_valid["eligibility/criteria/textblock"].fillna("unknown").tolist()
    )
    ...
    valid_dataset = Trial_Dataset_tabular(
        valid_nctid_lst,
        valid_label_lst,
        valid_smiles_lst,
        valid_icdcode_lst,
    ...
    test_criteria_lst = (
        X_test["eligibility/criteria/textblock"].fillna("unknown").tolist()
    )
    ...
    test_dataset = Trial_Dataset_tabular(
        test_nctid_lst,
        test_label_lst,
        test_smiles_lst,
        test_icdcode_lst,
    ...
    return (
        ...
        test_loader,
        0,
        len(numerical_features + hot_features),
    )
...
def mortality_rate_yn(phase):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=32, shuffle=True, collate_fn=trial_tabular_collate_fn
    )
    ...
    valid_loader = DataLoader(
        valid_dataset, batch_size=32, shuffle=False, collate_fn=trial_tabular_collate_fn
    )
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=32, shuffle=False, collate_fn=trial_tabular_collate_fn
    )
...
def dose(phase):
    ...
    train_dataset = Dose_dataset(
        train_nctid_lst, train_label_lst, train_smiles_lst, train_mesh_lst
    )
    train_loader = DataLoader(
        train_dataset, batch_size=32, shuffle=True, collate_fn=dose_collate_fn
    )
    ...
    valid_dataset = Dose_dataset(
        valid_nctid_lst, valid_label_lst, valid_smiles_lst, valid_mesh_lst
    )
    valid_loader = DataLoader(
        valid_dataset, batch_size=32, shuffle=False, collate_fn=dose_collate_fn
    )
    ...
    test_dataset = Dose_dataset(
        test_nctid_lst, test_label_lst, test_smiles_lst, test_mesh_lst
    )
    test_loader = DataLoader(
        test_dataset, batch_size=32, shuffle=False, collate_fn=dose_collate_fn
    )
