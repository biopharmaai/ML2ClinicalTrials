...
def Read_from_csv(target, phase):
    if "mortality_rate" in target:
        rootdir = "data/mortality-event-prediction"
    elif "serious_adverse_rate" in target:
...
def csv_ours_feature_2_dataloader(
    csvfile, shuffle, batch_size, phase="Phase 1", label="outcome"
):
    ...
    data_loader = data.DataLoader(
        dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=trial_collate_fn
    )
    return data_loader
...
def csv_three_feature_2_complete_dataloader(csvfile, shuffle, batch_size):
    ...
    dataset = Trial_Dataset_Complete(
        nctid_lst,
        status_lst,
        why_stop_lst,
        label_lst,
    ...
    data_loader = data.DataLoader(
        ...
        shuffle=shuffle,
        collate_fn=trial_complete_collate_fn,
    )
    return data_loader
...
def generate_admet_dataloader_lst(batch_size):
    ...
    for i, name in enumerate(name_lst):
        ...
        train_dataset = ADMET_Dataset(
            smiles_lst=train_smiles_lst, label_lst=train_label_lst
        )
        test_dataset = ADMET_Dataset(
            smiles_lst=test_smiles_lst, label_lst=test_label_lst
        )
