...
def download_all_data(save_path):
    ...
    url = "https://zenodo.org/records/15455785/files/all_task.zip?download=1"
...
def load_data(task, phase, data_format="df"):
    ...
    if data_format == "dl":
        ...
        train_loader, valid_loader, test_loader, num_classes, tabular_input_dim = (
            get_dataloader(task, phase)
        )
        train_df, valid_df, test_df = (
            data_loader_to_frame(train_loader, task),
            data_loader_to_frame(valid_loader, task),
            data_loader_to_frame(test_loader, task),
        )
...
if __name__ == "__main__":
    ...
    train_loader, valid_loader, test_loader, num_classes, tabular_input_dim = load_data(
        task, phase, data_format="dl"
    )
